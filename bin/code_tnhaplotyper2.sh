...
grep -v "GRCh38_GIABv3_no_alt_analysis_set_maskedGRC_decoys_MAP2K3_KMT2C_KCNJ18_noChr" "genome/GRCh38_GIABv3_no_alt_analysis_set_maskedGRC_decoys_MAP2K3_KMT2C_KCNJ18_noChr.fa.fai"|grep -v "EBV"|grep -v "decoy"|awk 'BEGIN{OFS="\t"}{print $1,0,$2}' > ignore_decoy.bed
#tnhaplotyper2
$SENTIEON_APP driver -t 36 -r genome/hs37d5.fa --interval ignore_decoy.bed -i ${bam} --algo QualCal -k resources/Homo_sapiens_assembly38.known_indels.vcf.gz -k resources/Mills_and_1000G_gold_standard.indels.hg38.vcf.gz -k resources/Homo_sapiens_assembly38.dbsnp138.vcf.gz tumor_recal_data_Sentieon.table
